...
class LabelHandler {
  ...
  async getIssues(repo) {
    const { data } = await this[request](`/repos/${repo}/issues`, 'GET');
    return JSON.parse(data);
  }
  ...
  async deleteIn(repo, labelName) {
    ...
    for (const issue of issues) {
      const issueLabels = issue.labels.map(label => label.url.split('/').pop());
      for (const label of labelName) {
        if (issueLabels.includes(label)) {
          throw new Error(
            `Tried to delete label '${label}',` +
              ` but issue '${issue.html_url}' has it`
          );
        }
      }
    }
