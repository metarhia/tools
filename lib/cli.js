...
class Cli {
  ...
  [buildHelp]() {
    ...
    for (const [name, opt] of this.options) {
      ...
      const optionName =
        '--' + toSpinalCase(name) + (opt.alias ? `, -${opt.alias}` : '');
  ...
  conflict(a, b) {
    a = toCamelCase(a);
    b = toCamelCase(b);
    if (this.conflicts.has(a)) this.conflicts.get(a).push(b);
    else this.conflicts.set(a, [b]);
    return this;
  }
  ...
  parse(argv, cb) {
    ...
    this.conflicts.forEach((bArr, a) => {
      if (args[a] !== undefined) {
        bArr.forEach(b => {
          if (args[b] !== undefined) {
            console.error(`Options '${a}' and '${b}' are in conflict`);
            this.showHelp();
            process.exit(1);
          }
        });
